        ...
        <div className="flex gap-6">
          ...
          <div className="w-24 h-24 bg-muted rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center">
            {(() => {
              ...
              const displayImage = featuredImage || property.images?.[0];
              ...
              if (displayImage?.imageUrl) {
                return (
                  <img
                    ...
                    onError={(e) => {
                      console.error('[PropertyDetail] Image failed to load:', displayImage.imageUrl);
                      // Fallback to icon if image fails to load
                      const img = e.target as HTMLImageElement;
                      img.style.display = 'none';
                      const parent = img.parentElement;
                      if (parent) {
                        parent.innerHTML = '<svg class="h-12 w-12 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>';
                      }
                    }}
                  />
                );
